...
![Correlation](../results/figures/correlation_plot.png){#fig-correlation}

Physicochemical Feature Distributions and Feature Correlation
:::
...
Although our model performed well, there are several possible limitations of linear models that may be negatively impacting its performeance. The first of which is the potential of the explanatory features having a non-linear relationship with the target. If this is the case then it would undoubtedly impact the performance of the model. Another potential limitation that arises due to the linearity of the model is the unboundedness of the output. Although our model was trained on examples that had discrete integer quality scores from 1 to 10, our model has the capability of predicting scores beyond this range. We still opted for a linear model with these limitations in mind, as the interpretability of results was a priority.
